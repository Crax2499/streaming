			...
			/*=============================================
  			...
  			if(isset($_GET["perfilOculto"]) || $_GET["perfilOculto"] == "Administrador" || $_GET["perfilOculto"] == "GAdministrador"){
				...
				if($fecha == $respuesta[$i]["fecha_termino"]){
					...
					$buttonEstado = "<a href='https://api.whatsapp.com/send?phone=".$respuesta[$i]["telefono"]."&text=%2ABienvenid%40%20a%20FAV%20Stream%20EC%20🍿🎬%2A%0A_%2A📲Plataforma:%2A_%20".$streaming["nombre"]."%0A_%2A📩Correo:%2A_%20".urlencode($correo)."%0A_%2A🔓Clave:%2A_%20".urlencode($cuenta["password"])."%0A_%2A📅Vence:%2A_%20".$respuesta[$i]["fecha_termino"]."%0A%0A-%20♻%20Si%20desea%20renovar%20cancelar%20unos%20dias%20antes%0A-%20⏯️%20Disfrute%20de%20nuestro%20servicio%20😊' target='_blank'><button class='btn btn-success btn-sm'>Cuenta Activa</button></a>";
				}
			}else{
				if($fecha < $respuesta[$i]["fecha_termino"]){
